...
- A jupyterlab extension, consisting of a react/TS frontend and a python backend.
  - The backend is used to
    - forward calls to the InvenioRDM API proxy
...
- An isolated InvenioRDM API Proxy that forwards the users requests to the InvenioRDM API and can store the InvenioRDM access tokens needed for that securely.

## Quirks of the System Architecture and why they exist

### Why do we need an extra proxy for the InvenioRDM API requests and token storage?

The InvenioRDM API requires the caller to send a InvenioRDM access token to execute requests on behalf of a user, e.g. for publishing record drafts as the user or reading files the user, but not everyone has access to. We need to store this token somewhere securely.
...
The extension backend is also not the right place. It is run in the same environment and with the same OS user as the jupyter server. This is not a problem if there is only one server per user, which is the recommended way to deploy a production Jupyter system. E.g. JupyterHub spins up a jupyter server per user by default and they are run in isolation. However, Juypter 5.0 introduces [Sharing Access to your Server](https://jupyterhub.readthedocs.io/en/stable/tutorial/sharing.html) for collaborative working, which can allow multiple users to execute code in the same server environment. Since this allows other users to read out environment variables and files, storing the single-user InvenioRDM access token here is insecure and shares it with the other users. Since this extension should be independent of the deployment configuration of the notebook execution backend behind JupyterLab, we should not store the access tokens there.

Therefore, a third, isolated component is needed to store the access token securely. Since the access tokens should not leave this environment for the API calls to be executed, they should be executed from the same place, making this a proxy for the InvenioRDM API. The proxy should be run in an isolated environment away from the users servers and can be used globally for all users using the deployed instance.

#### Why not use OAuth refresh tokens instead of having this proxy? Isn't the session cookie just as long-lived as a refresh token?

The first reason for why we need the proxy server is that the inveniordm tokens are so long lived (one year according to privacy policy) and the docs do not really promise refresh tokens. a more typical situation would be to have an access token with a shorter lifespan, e.g. 10min, and a refresh token with lifespan 30days. When the access token expires, the refresh token could be automatically presented to inveniordm to get a new short lived access token. Unfortunately, inveniordm oauth/ the underlying [invenio oauth](https://github.com/inveniosoftware/flask-oauthlib-invenio/tree/master) does not really seem to reliably support refresh tokens.

For the API Proxy, we currently have a session cookie that is stored in the browser and used to authenticate against the proxy server (lifetime 14 days). One could argue that this is very similar to a OAuth refresh token because it has a similar lifetime and can also be rotated, and if we just added support for refresh tokens to inveniordm, we would not need the proxy anymore.
But even if we could use refresh tokens, authenticating against a proxy with a short lived session cookie instead still has its benefits:

- Refresh token allows to create arbitrary new access tokens once it is stolen, with any inveniordm operation allowlist, while with the api proxy, we never even get to see the actual access token.
- The session cookie can be marked httponly and not read out directly from malicious scripts. refresh tokens can be stolen and used from any kind of environment even after jupyterlab is closed.
- If the session cookie is abused, the session can be revoked easily by the user simply by clicking "log out". If a refresh token is stolen, InvenioRDM needs to provide appropriate functionality to revoke that token and probably also all access tokens generated by that refresh token. Even if it did, this would be less intuitive and easy.

#### Interacting with the users notebook in the Frontend code: How does this align with "Thin Client"?

When a file is downloaded from inveniordm, the user can click a button to add the path to this file as a hardcoded string into the current jupyter notebook, so they can use it without having to find the location themselves.
...
#### Supporting both Production and Sandbox InvenioRDM: Isnt this an inconvenient feature to support long term?

There are two deployed instances of InvenioRDM, inveniordm.org and sandbox.inveniordm.org.

Developers of this extension and inveniordm might want to use the Sandbox instance in the extension instead of Production. This is implemented in the following ways:
...
Other times, SSE are only used to notify the client that a certain information has changed and the value cached by the client might be invalid ("invalidation events"), not including the actual information in the event. For example, the SSE informing the client that the login status has changed, is used by the client to trigger multiple requests, e.g. to get the username and to request which inveniordm instance is currently used.
...
This matters because browser cookies, JupyterLab frontend state, and the OAuth authorization code flow are all tied to the URL that was used. Mixing URLs during login can lead to confusing `invalid_grant` errors from InvenioRDM, because the authorization code is only valid for the exact client and redirect URI used when the login flow was started. This is a general OAuth requirement/ issue and we therefore cannot get rid of the underlying problem, therefore:

For production deployments, there should be one canonical public URL for the Jupyter service, and the OAuth callback URL registered in InvenioRDM should use that canonical URL. Other hostnames or aliases should redirect to the canonical URL before OAuth login starts. For local development, avoid switching between `localhost` and `127.0.0.1` in the same browser session.
