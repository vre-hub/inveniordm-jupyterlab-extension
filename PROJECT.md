...
The first reason for why we need the proxy server is that the zenodo tokens are so long lived (one year according to privacy policy) and the docs do not really promise refresh tokens. a more typical situation would be to have an access token with a shorter lifespan, e.g. 10min, and a refresh token with lifespan 30days. When the access token expires, the refresh token could be automatically presented to zenodo to get a new short lived access token. Unfortunately, zenodo oauth/ the underlying [invenio oauth](https://github.com/inveniosoftware/flask-oauthlib-invenio/tree/master) does not really seem to reliably support refresh tokens.

For the API Proxy, we currently have a session cookie that is stored in the browser and used to authenticate against the proxy server (lifetime 14 days). One could argue that this is very similar to a OAuth refresh token because it has a similar lifetime and can also be rotated, and if we just added support for refresh tokens to zenodo, we would not need the proxy anymore.
...
- Refresh token allows to create arbitrary new access tokens once it is stolen, with any zenodo operation allowlist, while with the api proxy, we never even get to see the actual access token.
...
- If the session cookie is abused, the session can be revoked easily by the user simply by clicking "log out". If a refresh token is stolen, Zenodo needs to provide appropriate functionality to revoke that token and probably also all access tokens generated by that refresh token. Even if it did, this would be less intuitive and easy.
