...
from inveniordm_api_proxy.app import create_app
from inveniordm_api_proxy.config import Config
from inveniordm_api_proxy.types import ProxyState
from inveniordm_auth.token_store import MultiTokenStore, StoredToken
...
class TestApiProxyHandler(AsyncHTTPTestCase):
    def get_app(self):
        return create_app(
            Config(
                inveniordm_base_url="https://inveniordm.example",
                client_id="client-id",
                session_cookie_name="inveniordm_sandbox_proxy_session",
            ),
    ...
    def auth_headers(self) -> dict[str, str]:
        return {"Cookie": "inveniordm_sandbox_proxy_session=session"}
    ...
    def test_preserves_percent_encoded_file_path(self):
        ...
        assert upstream_urls == [
            "https://inveniordm.example/api/records/541036/draft/files/"
            "FINAL%20REPORT_Results%20%281%29.pdf?download=1"
        ]
    ...
    def test_upstream_failure_does_not_block_incoming_upload(self):
        with patch(
            "inveniordm_api_proxy.api_proxy_handler.urlopen",
