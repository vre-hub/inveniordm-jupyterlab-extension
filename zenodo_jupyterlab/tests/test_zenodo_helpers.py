...
def test_include_zenodo_file_returns_entries_as_map(monkeypatch):
    item = {
        "id": "record-1",
    ...
    zenodo_helpers.include_zenodo_file(
        item,
...
def test_include_zenodo_file_without_files_link_does_nothing(monkeypatch):
...
def test_include_zenodo_file_if_draft_or_restricted_includes_files(
    monkeypatch, record
):
    calls = []
    monkeypatch.setattr(
    ...
    zenodo_helpers.include_zenodo_file_if_draft_or_restricted(
        record,
        base_url="https://zenodo.org",
        headers={"Authorization": "x"},
    )
...
def test_include_zenodo_file_if_draft_or_restricted_skips_public_records(
    monkeypatch, record
):
    monkeypatch.setattr(
        zenodo_helpers,
