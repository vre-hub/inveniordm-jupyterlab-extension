...
import {
  getInvenioRDMRecordVariant,
  listInvenioRDMRecordVersions,
  InvenioRDMRecordData,
  InvenioRDMRecordIdentifier,
  InvenioRDMRecordVersion,
  InvenioRDMRecordVersionsChangedEventData,
...
): InvenioRDMRecordVersion | undefined {
  return (
    versions.find(
...
export function useInvenioRDMVersionedRecord({
  ...
  const [recordIdentifier, setRecordIdentifier] =
    React.useState<InvenioRDMRecordIdentifier>(initialRecordIdentifier);
  ...
  const loadRecord = React.useCallback(
    async (
      ...
      try {
        ...
        // Recover from a 404 error when the record is a draft that has been discarded from external sources (e.g. InvenioRDM web interface)
        if (
          ...
          if (nextVersion) {
            ...
            setRecord(
              await getInvenioRDMRecordVariant(serverSettings, nextIdentifier)
            );
          } else {
  ...
  useEventListener('record.versions.changed', event => {
    const eventData = event.data as
      InvenioRDMRecordVersionsChangedEventData | undefined;
